...
window.onload = () => {
    ...
    const buttons = {
        ...
        transfer: async () => {
            ...
            else if (status == 405) Swal.fire({ title: 'Erro', text: 'Você só pode transferir moedas entre contas do PlainPost!', icon: 'error' });
    ...
    document.getElementById("inbox").addEventListener("click", async function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const { value: resposta, isConfirmed } = await Swal.fire({
                    ...
                    inputValidator: (value) => {
                        if (!value.trim()) return "A mensagem não pode estar vazia!";
                    }
                });

                if (isConfirmed) {
                    try {
                        const res = await fetchRequest("send", { to: autor, subject, content: resposta.trim() });
